...
- 🎨 **Full Excalidraw Integration**: Drawing, shapes, text, handwriting, light/dark themes, and canvas zoom/pan.
- 💾 **Robust Auto-Save & Zero Data Loss**:
  - 3-second debounced server auto-saving.
  - 1ms immediate browser `localStorage` draft caching.
  - Page unload (`beforeunload`) keepalive auto-flush.
...
```text
data/
├── app.db               # SQLite database (Node 24 built-in node:sqlite with WAL)
├── attachments/         # Embedded images and scene asset files
└── thumbnails/          # 400px PNG document preview thumbnails
```
