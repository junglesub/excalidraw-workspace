...
**Date:** 2026-09-01
**Status:** Implemented; merge review complete; manual browser verification pending
...
A copied-storage context (opener-created or duplicated) has its own `window.name` id, so it cannot use the credential-proven same-context rotation path. A different context remains held while the current heartbeat is fresh, but the same user may recover directly once that heartbeat has been stale longer than the 10-second forced-takeover window, provided no valid takeover is already pending. Different users still require takeover or normal lease expiry.
...
```typescript
type LeaseState = "acquired" | "held" | "takeover_pending" | "takeover_in_progress" | "released" | "transferred" | "lost";
