...
- [x] Strict Mode false-conflict fix: initial lease effect now shares its acquire promise/candidate credentials across mount/unmount/replay (3 refs: `initialAcquirePromiseRef`, `initialAcquireCandidateRef`, `initialAcquireFinalizedRef`). Replay mounts reuse the same in-flight request and candidate instead of generating a second; only the active effect finalizes. Cleanup never releases via replay; genuine unmount release preserved. Test at `tests/initial_acquire_strict.test.ts` (1: concurrent-dual-request timeline proves bug, server-side `acquireEditLease`). Preserves credential-proven reload, generation fencing, compare-and-clear, takeover, VIEWER/admin/recovery.
- [x] Validation (2026-09-01): focused lease tests pass (reentry 8, edit_lease 16, edit_lease_route 7, client_edit_lease 13, initial_acquire_strict 1); full `npm test` 187 tests across 23 files all pass, `npm run typecheck` clean, `git diff --check 078659a..HEAD` 0
