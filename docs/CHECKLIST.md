...
- [x] Strict Mode false-conflict fix: initial lease effect uses `InitialLeaseCoordinator` (`src/lib/client_edit_lease.ts`) to share in-flight candidate credentials and acquire promise across React Strict Mode mount/unmount/replay. Replay mounts subscribe to the in-flight request without launching duplicate acquires; only the active non-cancelled subscriber finalizes. Strict Mode cleanup followed by replay does not release the adopted result; genuine unmount before resolution releases an acquired result exactly once without finalization; genuine unmount after finalization releases once; coordinator state is scoped and disposed per `docId`. Production-connected tests at `tests/initial_acquire_strict.test.ts` (4 tests). Preserves credential-proven reload, generation fencing, compare-and-clear, takeover, VIEWER/admin/recovery.
- [x] Validation (2026-09-01): focused lease tests pass (reentry 8, edit_lease 16, edit_lease_route 7, client_edit_lease 13, initial_acquire_strict 4); full `npm test` 190 tests across 23 files all pass, `npm run typecheck` clean, `git diff --check 078659a..HEAD` 0
