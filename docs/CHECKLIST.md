...
- [x] Root cause: `sessionStorage` clientId is per-tab and survives reload/navigation, while each page instance generates a fresh lease token (`EditorClient.tsx`); the server's idempotent-acquire branch only matches exact credentials (user + clientId + token), so a same-tab reload presented a new token and fell through to the held/conflict gate even though the heartbeat was fresh — "already being edited" with no other editor
- [x] Minimal fix in `src/lib/edit_lease.ts` `acquireEditLease`: same user + same per-tab clientId + different token = same editor context re-entry → immediate re-acquire, rotating the token and advancing generation so the previous page instance's in-flight requests (including a late best-effort pagehide release) are fenced out; blocked while a structurally valid pending takeover exists
- [x] Global one-editor-per-document invariant preserved: a second tab, new window, or relaunched browser always presents a different clientId and remains behind the conflict modal + takeover UX; the 10s stale-heartbeat re-acquire (> `TAKEOVER_TIMEOUT_MS`, consistent with forced-takeover semantics) remains the only no-takeover recovery for those cases
- [x] Server-side indistinguishability limit: a full browser close / new window of the same user is indistinguishable from a second live tab until the holder heartbeat goes stale; those cases intentionally keep the conflict/takeover UX for up to 10s
- [x] Regression `tests/reentry_lease.test.ts` (6 tests): immediate same-tab re-entry acquires (generation+1) and fences stale-token heartbeat and late pagehide release; same-user second live tab held with holder lease untouched; different user held; stale-heartbeat different-clientId recovery; pending takeover not clobbered; expired lease acquires; `tests/edit_lease.test.ts` held-case updated to same-tab re-acquire + different-clientId conflict
