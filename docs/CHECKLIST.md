...
- [x] Root cause: `sessionStorage` clientId survives reload/navigation while each page instance generates a fresh lease token (`EditorClient.tsx`); the server's idempotent-acquire branch only matches exact credentials (user + clientId + token), so a same-tab reload presented a new token and fell through to the held/conflict gate even though the heartbeat was fresh — "already being edited" with no other editor
- [x] Platform evidence: MDN sessionStorage — "If the page has an opener, the sessionStorage is initially a copy of the opener's sessionStorage object" (duplicated tabs copy it too). So clientId is NOT guaranteed unique per tab; the same-clientId branch in 46f9a8e/b80fe11 could rotate the lease of a live editor context. Web Locks (MDN) are held by the live document, released on unload, and never copied to opener/duplicate contexts — the correct per-context liveness primitive
- [x] Fix: server rotation for same-user re-entry requires an explicit `reentry: true` attestation AND matching clientId (`src/lib/edit_lease.ts`, wired through `src/app/api/documents/[id]/lease/route.ts`); the client sets it only after winning a Web Locks liveness probe (`probeReentryLock`, retries absorb the reload unload-release race) and holds the per-context lock (`edit-lease-hold:{docId}:{clientId}`) while it owns the lease (`requestLeaseHold`, released on loss/unmount/pagehide). Token rotation + generation advance fence the previous page instance's in-flight requests (including a late pagehide release)
- [x] Safety: same-user concurrent tab with copied clientId (opener/duplicate) → probe denied → conflict modal + takeover UX, live lease untouched; different clientId → held until heartbeat stale (> `TAKEOVER_TIMEOUT_MS`) → stale recovery; pending takeover blocks all re-entry; no Web Locks support → safe-fail to conflict path (no immediate re-entry, no takeover-free rotation)
- [x] Regressions: `tests/reentry_lease.test.ts` (7 tests: attested same-tab re-entry with stale-token/late-release fencing; copied-clientId collision stays held with lease untouched; second live tab held; different user held; stale different-clientId recovery; pending takeover not clobbered; expired lease), `tests/edit_lease.test.ts` (attested re-acquire + different-clientId conflict), `tests/edit_lease_route.test.ts` (409 without attestation, 200 with), `tests/client_edit_lease.test.ts` (probe/hold/lock-name/flag transport)
