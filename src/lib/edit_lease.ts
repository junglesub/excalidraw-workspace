...
export function parseAndValidateLeaseCredentials(body: unknown): EditLeaseCredentials | null {
...
export function acquireEditLease(input: LeaseIdentityInput, now?: Date): LeaseResult {
  ...
  return transaction(() => {
    ...
    // Same-user recovery preserves fast re-entry without creating a separate recovery protocol:
    // exact prior credentials prove same-context reload, while a different context may recover
    // only after the holder heartbeat has been stale for the forced-takeover window. A live
    // pending takeover always wins and is never clobbered by this recovery path.
...
export function assertActiveEditLease(input: ActiveLeaseInput, now?: Date): void {
  const nowDate = now ?? new Date();
  // Mutation callers run this check inside the same BEGIN IMMEDIATE transaction as the write.
  validateWritePermission(input);
  const row = getLeaseRow(input.docId);
  if (!row || !row.holder_user_id) {
    throw new HttpError(409, "Editing lease was lost", "EDIT_LEASE_LOST");
